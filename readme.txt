...
* [ 機能追加 ] 投稿タイプ「給与テンプレート」を追加（給与明細の雛形として利用）
* [ 機能追加 ] 給与明細一覧画面に、給与テンプレートから複数スタッフ分の給与明細を下書きで一括登録するパネルを追加
* [ 機能追加 ] 給与テンプレート編集画面に「このテンプレで一括登録」ボタンを追加（一覧の一括登録パネルへテンプレ選択済み状態で遷移）
* [ 仕様変更 ] 給与明細のカスタムフィールド定義を給与テンプレートでも共通利用するように変更（給与テンプレートでは「スタッフ」「Staff No.」「発行済PDF」など給与明細固有の項目は非表示）
