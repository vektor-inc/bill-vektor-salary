...
### 目的
- 給与明細投稿（`salary`）に対して、対象スタッフへメール通知を送信できるようにする。
- フェーズ2で発行したPDFを添付して送信できるようにする。
- 送信結果（成功/失敗）を投稿ごとに追跡できるようにする。

### フェーズ3 確定事項
- 送信トリガーは当面「手動送信」（管理画面ボタン）とする。
- 送信単位は「給与明細1件ずつ」とし、編集画面で開いている `salary` 投稿のみ送信対象にする。
- 既存の「PDF発行」ボタンの直下に「送信プレビュー」ボタンを配置する。
- 「送信プレビュー」押下時にプレビュー用ポップアップを表示し、ポップアップ内の「メールを送信する」ボタン押下で送信実行する。
- メール本文はフェーズ1の `bvsl_build_salary_message( $post_id )` を利用する。
- 添付ファイルはフェーズ2の `bvsl_pdf_history` 先頭（最新PDF）を既定値として利用する。
- PDFが未発行の場合は、メール送信処理の中で対象明細のPDFを自動発行してから添付する。
- 送信先（`to`）は、給与明細に紐づくスタッフ情報に保存されているメールアドレスを使用する。
- 送信先メールアドレスは、スタッフ基本情報のカスタムフィールド「メールアドレス」入力欄の値を使用する。
- メール本文の宛名は、選択中スタッフ名に `様` を付与した形式（例: `山田 太郎 様`）で出力する。
- 固定署名機能はフェーズ3初期実装では提供しない。
- 送信元（From/Reply-To）はWordPress既定設定を使用し、フェーズ3では専用設定画面を作成しない。
- 送信先メールアドレスが取得できない場合は送信不可（エラー表示）とする。
- メール送信履歴は `salary` 投稿メタに保存し、管理画面で確認できるようにする。
...
- `inc/salary-mail.php`
  - メール送信の中心ロジック
  - `bvsl_send_salary_mail( $post_id, $args = array() )` を定義
  - 送信履歴メタ更新・入力バリデーション・`wp_mail()` 実行を担当
- `assets/js/admin-salary-mail.js`（または既存 `admin-salary-pdf.js` へ追記）
  - 「送信プレビュー」ボタンのクリックハンドラ
  - プレビューポップアップ描画・確認後の送信Ajax・レスポンス反映

### 変更ファイル
- `bill-vektor-salary.php`
  - `inc/salary-mail.php` の `require_once` 追加
  - Ajax ハンドラ登録：`wp_ajax_bvsl_send_salary_mail`
  - 管理画面へ nonce と必要データを連携
- `inc/custom-field-setting/custom-field-salary-normal.php`（必要時）
  - 送信先メールアドレス表示欄/補助情報の追加
- `docs/spec-send-mail.md`
  - 本フェーズ仕様を更新（本項）
...
| キー | 対象 | 値型 | 内容 |
|------|------|------|------|
| `bvsl_mail_history` | post type `salary` | 配列（シリアライズ） | 送信履歴（成功/失敗含む） |
| `bvsl_last_mail_status` | post type `salary` | string | 直近送信ステータス（`success` / `failed`） |
| `bvsl_last_mail_sent_at` | post type `salary` | string | 直近送信日時（`Y-m-d H:i:s`） |

### `bvsl_mail_history` の配列構造
...
### 「PDF発行」メタボックス内のメール送信UI（サイドバー）
- 表示位置：既存「PDF発行」メタボックス（`side` / `high`）内
- 表示内容：
  - 送信先メールアドレス（読み取り専用表示）
  - 件名（初期値あり、編集可）
  - 添付PDF選択（初期値：最新PDF）
  - 「送信プレビュー」ボタン（「PDF発行」ボタンの直下）
- 新規投稿（未保存）または送信先不明時は「送信プレビュー」ボタンを無効化する。

### 送信プレビューポップアップ
- 表示契機：「送信プレビュー」ボタン押下
- 表示内容：
  - 件名プレビュー
  - 本文プレビュー（先頭に `{スタッフ名} 様`、続けて `bvsl_build_salary_message( $post_id )` 結果）
  - 確認文言：`この内容でメールを送信しますか`
  - 「メールを送信する」ボタン
- 「メールを送信する」押下時に送信Ajaxを実行する。

### 送信履歴テーブル
- 表示位置：給与明細編集画面内（既存PDF管理テーブルの下を想定）
- 表示項目：
  - 送信日時
  - 送信先
  - 件名
...
### 送信元データ
- 宛先（`to`）：給与明細に紐づくスタッフ情報のメールアドレスを取得して使用する
- 件名（`subject`）：`【給与明細】 {支給分ターム名} {スタッフ名} 様` の形式で生成する（半角スペース区切り）
- 本文（`message`）：公開側と同じ組み立てルールで生成し、`bvsl_build_salary_message( $post_id )` の返り値を使用する
- 添付（`attachments`）：選択された `attachment_id` から実ファイルパスへ解決。未指定または最新PDFが未発行の場合は送信前にPDFを自動発行して添付する
- 送信元（`headers`）：WordPress既定の送信元設定を利用する

### 本文フォーマット
- 先頭に宛名を配置する（`{スタッフ名} 様`）
- 宛名の後に空行を入れ、本文として `bvsl_build_salary_message( $post_id )` の結果を配置する
- これにより、給与明細の「メッセージ」入力欄と「メッセージ構成」指定（1/2/3）を公開側表示と同一ロジックで反映する

### 送信処理
1. nonce と権限（`current_user_can( 'edit_post', $post_id )`）を検証
2. `post_id` と送信先メールアドレスを検証
3. 添付指定がある場合は `get_attached_file()` で実在確認。添付が存在しない場合は対象PDFを自動発行する
4. プレビューポップアップでの最終確認後、`wp_mail( $to, $subject, $message, $headers, $attachments )` を実行
5. 結果を `bvsl_mail_history` へ保存（成功/失敗どちらも保存）
6. 直近ステータスを `bvsl_last_mail_status` / `bvsl_last_mail_sent_at` に反映

### Ajax エンドポイント：メール送信
- アクション名：`bvsl_send_salary_mail`
- リクエスト：`POST`
  - `nonce`
  - `post_id`
  - `subject`
  - `attachment_id`（任意）
- レスポンス（成功）：`{ sent: true, sent_at: "...", to: "...", subject: "..." }`
- レスポンス（失敗）：`{ sent: false, message: "エラー内容" }`
- セキュリティ：
  - `check_ajax_referer( 'bvsl_send_salary_mail_nonce', 'nonce' )`
  - `current_user_can( 'edit_post', $post_id )`
  - `sanitize_text_field()` / `sanitize_email()` による入力サニタイズ

## 実装順序（フェーズ3）

1. `inc/salary-mail.php` を作成し、送信ロジックと履歴保存処理を実装
2. `bill-vektor-salary.php` に読み込み・Ajaxフック・nonce受け渡しを追加
3. 管理画面メタボックス（送信UI）を実装
4. 管理画面JSで送信リクエストと結果反映を実装
5. 送信履歴テーブルを実装
6. 動作確認（成功ケース/失敗ケース/権限チェック）

## 確認観点（フェーズ3）

- ポップアップ内の「メールを送信する」ボタン押下で `wp_mail()` が実行され、宛先にメールが届く
- 「送信プレビュー」押下でポップアップに件名/本文が表示される
- 件名が `【給与明細】 {支給分ターム名} {スタッフ名} 様` 形式で表示される
- ポップアップに `この内容でメールを送信しますか` と `メールを送信する` ボタンが表示される
- 送信対象は編集画面で開いている給与明細1件のみで、他明細は送信されない
- 「PDF発行」ボタンの直下に「送信プレビュー」ボタンが表示される
- 本文先頭に `{スタッフ名} 様` が入り、続く本文が公開側と同じメッセージ構成で出力される
- 最新PDFを添付した場合、添付ファイルが開ける
- PDF未発行の明細でも、送信時にPDFが自動発行されて添付送信される
- 送信失敗時に管理画面へエラー内容が表示される
- 成功/失敗いずれも履歴が `bvsl_mail_history` に保存される
- 権限のないユーザーは送信できない
- 宛先不正・添付ファイル未存在時に安全に失敗する

## 未検討・要確認（フェーズ3）

- HTMLメール化の要否（当面はプレーンテキスト前提）
- 自動送信（公開時/特定日時）の導入要否（フェーズ4候補）
